...
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), ".pi", "agent", "pi-system-theme.json");
...
function mergeConfig(base: Config, rawConfig: RawConfig | undefined): Config {
    ...
    const commandTimeoutValue = toPositiveInteger(rawConfig.commandTimeoutMs);

    const pollMs = pollMsValue ? Math.max(pollMsValue, MIN_POLL_MS) : base.pollMs;
    const commandTimeoutMs = commandTimeoutValue
        ? Math.max(commandTimeoutValue, MIN_TIMEOUT_MS)
...
function getOverrides(config: Config): Record<string, string | number> {
    ...
    if (config.commandTimeoutMs !== DEFAULT_CONFIG.commandTimeoutMs) {
        overrides.commandTimeoutMs = config.commandTimeoutMs;
    }
...
async function detectAppearance(commandTimeoutMs: number): Promise<Appearance> {
    try {
...
export default function systemThemeExtension(pi: ExtensionAPI): void {
    ...
    async function syncTheme(ctx: ExtensionContext): Promise<void> {
        ...
        try {
            const appearance = await detectAppearance(activeConfig.commandTimeoutMs);
    ...
    pi.registerCommand("system-theme", {
        ...
        handler: async (_args, ctx) => {
            ...
            const commandTimeoutMs = await promptIntegerSetting(
                ctx,
                "Detection command timeout (ms)",
                activeConfig.commandTimeoutMs,
                MIN_TIMEOUT_MS,
            );
            if (commandTimeoutMs === undefined) return;
